...
let REPOSITORYLIST = {};   // already declared by puls
let knownmodules = {};
let repolistener;
...
class RepoLoader extends Loader {
    ...
    async doFetch(request) {
        ...
        // debuglog("+> fetch", request.url, "referrer version", rversion);
        let response;
        response = await this.fromCache(request, rversion);
        if (response) {
            // debuglog("+> found in cache", request.url);
        } else {
            response = await this.loadAndCache(request, rversion);
            if (response) {
                // debuglog("+> load archive and cache", request.url);
            } else {
                response = this.notFound(request);
                debuglog("+> not found", request.url);
            }
        }
        /*
            ...
            try {
                ...
                if (await isModuleInCache(reponame, modulename, moduleversion)) { /*isAnySourceLoaded(sources)*/
                    if (this.isCaching) {
                        archiveQ.push(async () => {
                            try {
                                // debuglog("+> get module while caching, fetching again", modulename, moduleversion);
                       ...
                       // debuglog("+> module was loaded, fetching again", modulename, moduleversion);
                        const cache    = await openRepoCache(modulespec.repo);
                        const response = await cache.match(reference);
                        resolve(response);
                    }
                    ...
                    debuglog("+> load source for module", modulename, moduleversion);
