 ...
 *      - the pubkey must be retrieved from a (global) registry, don't use any keys provided with the package!
...
function adjustVersion(request, rversion) {
    if (!SUPPORT_VERSIONS || request.url.indexOf('@') > -1) return request.url;  // todo [OPEN]: better semantic parsing, '@' may occur in the filename also
...
async function isModuleInCache(reponame, modulename, moduleversion) {
    ...
    let res = await cache.match(SUPPORT_VERSIONS ? `/${modulename}@${moduleversion}` : `/${modulename}`);
...
class RepoLoader extends Loader {
    ...
    async doFetch(request) {
        let rversion; // = '';
        if (SUPPORT_VERSIONS) {
            rversion = getModule4ReferrerVersion(request);
    ...
    async fromCache(request, rversion) {
        let reference = adjustVersion(request, rversion);
        if (!reference) return;
        let i = reference.indexOf('?');
        if (i > -1) reference = reference.substring(0, i);
        i = reference.indexOf('#');
        if (i > -1) reference = reference.substring(0, i);
/*
...
*/
    ...
    /*async*/ loadAndCache(request, rversion) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const reference = SUPPORT_VERSIONS ? pathname.indexOf('@') > -1 ? pathname : pathname.replace(`/${modulename}`, `/${modulename}@${rversion}`) : `/${modulename}`; // todo [OPEN]: semantic parsing. the '@' may also be in a filename
    ...
    async deflateAndCache(reference, archive, modulespec, version) {
        ...
        for await (const entry of entries) {
            ...
            hasIndex = entry.filename === 'index.mjs';
        }
        // add default entry for index.mjs
        if (!hasIndex) {
            let response = new Response('export default {};', {
                headers: {
    ...
    maintainStructure(location, zipentry, structure, version) {
        ...
        if (SUPPORT_VERSIONS) entries.version = version;
    ...
    buildLocation(filename, version) {
        if (SUPPORT_VERSIONS && (!version || version === 'undefined')) {
            // debugger;
        }
        filename = '/' + filename;
        const modulename = getModuleName(filename);
        return SUPPORT_VERSIONS ? filename.replace(versionRegex(modulename), `/${modulename}@${version}`) : filename;
    }
