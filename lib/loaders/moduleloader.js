...
class ModuleLoader extends Loader {
    ...
    async doFetch(request) {
        ...
        if (response) {
            debuglogML(">> ModuleLoader found in cache", request.url);
        } else {
        //     response = await this.loadAndCache(request);
        //     if (response) {
        //         debuglogML(">> ModuleLoader load archive and cache", request.url);
        //     } else {
        //         response = this.notFound(request);
            debuglogML("## ModuleLoader not found", request.url);
        //     }
        }
        return response;
